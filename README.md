...
* Friday, June 27: Phase 1 of this assignment is posted
* Wednesday, July 2: Phase 2 will be posted. 
* Thursday, July 3: Teams complete Phase 2 to add a new feature. This can be done independently from Phase 1.
* Thursday, July 10: Both Phase 1 and Phase 2 are due on MarkUs. The submission instructions will be posted on July 3rd before the lab, along with the self-testing on MarkUs.
...
### UML Diagram
You can generate a UML diagram of the code by right-clicking on the `src\main\java` folder in IntelliJ and selecting `Diagrams -> Show Diagram...`. This helps you understand the relationships between the classes/interfaces in the program.

Try to think about how the UML diagram relates to the Clean Architecture.
...
The program is currently connecting to an external API — the grade APIs from lab 3, but a different API endpoint for storing a password protected
...
version of the program's DAO instead. What is the advantage? You can focus on the logic first and then worry about integrating with real APIs. (Thanks to the DAO interface, you can switch between them easily!)
...
```
git branch <your_information>
git switch <your_information>
```
...
by typing `git status` and reading the first line of output. Use `git checkout` or `git switch` if you need to.
...
## Phase 2
The instructions for phase 2 will be posted as part of the July 3rd lab activity. Each group
